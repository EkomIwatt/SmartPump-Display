...
Numbers are **stable identifiers, not positions.** Resolved questions are deleted and their numbers
are never reused, so the sequence has holes — 5, 6, 14, 19, 20, 21 and 22 are under **Resolved** at the
bottom. Sections do not own contiguous ranges either, so find an item by its number here first.

**Hardware / firmware contract** — 1-4, 23-26
- **#1** Pulses per litre — the K-factor value itself; unmeasured, TEST-01 owns it
- **#2** Nozzle shutoff timeout — confirm 3 s, and whether it is operator-pushable
- **#3** Relay open-on-boot — does the app actively assert it
- **#4** USB serial protocol — ratify the 7a framing
...
- **#8** Price-per-litre push channel — FCM, pending ratification

**USSD / SMS (Flow 5), deferred** — 9-12
- **#9** GTBank SMS format · **#10** Pump SIM provisioning · **#11** USSD ref collisions · **#12** USSD code generation

**Operator config** — 13
- **#13** DeviceConfig schema — what `/config` carries; shutoff timeout still open

**UI / UX** — 15-18
- **#15** Idle screen attract loop · **#16** Custom amount entry · **#17** Error recovery copy · **#18** Cancellation paths

> **The protocol changes for Olonade are #23, #24 and #26.** They touch the same serial protocol and
> should go as one conversation rather than three — #26's `RLY:1:<pulses>` is also the signal #24
> needs for its session mark. #25 sits between them in the file but is an app-side recovery
...
- **#22 (Stuck fixed-flow sale)** — Resolved 2026-09-15, **Option 1** of [`OQ22_OPTIONS_DRAFT.md`](closed/OQ22_OPTIONS_DRAFT.md). A fixed sale (pre-pay, USSD, cash-fixed) had no exit but its target, and a power cycle restored the same stuck sale. Checking the code widened the question: besides a permanent link loss, a customer whose **tank fills before the target** strands the sale with the relay still on, and that needs no fault at all. **Decision: an attendant "End sale early" button**, behind the existing PIN, shown only during a fixed sale in the same slot as "End fill-up". It stops fuel and records `Complete` with **litres = what flowed, amount = what was paid**, audit note `Ended by attendant at X of Y L`; the customer screen says *"Sale ended early — X of Y L. Please see the attendant."* (no design screen covers it — deviation on record). **No automatic timeout:** a no-flow timer cannot tell a full tank from a pause, and ending a paid sale early by mistake is worse than a stuck screen; a link-loss-only timer (Option 3) can be added later if the field shows it is needed. Settling the shortfall is the attendant's (cash) or the backend's (digital, #7). `state-machine.md`'s Universal disconnect row, which said `Error(recoverable=true)` and would have written no audit row, is corrected.
